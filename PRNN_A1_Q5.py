# -*- coding: utf-8 -*-
"""
Created on Mon Feb 19 10:45:37 2024
...
#total data avilable = 70000
#Lets take 0.65 of it for training = 45000
A = 70000 #total data
B = 45000 #training data
C = A-B #test data
d = 25 #feature vector
...
file_path = r"C:\Users\sahap\OneDrive\Desktop\iisc materials\Mathematics\sem 4\PRNN\Asgns\Asgn 1\Data given\multi_class_classification_data_group_8_train.txt"
...
#Accuracy1 is  39.852%!
#At least better than random guessing which leads to 10% accuracy

#To plot CM
def plot_matrix_as_table(matrix, title="Matrix Table"):
    fig, ax = plt.subplots()
    ax.set_title(title)
    ax.axis('off')  # Hide axes
    table_data = []
    for row in matrix:
        table_data.append(['%.2f' % val for val in row])
    table = ax.table(cellText=table_data, loc='center', cellLoc='center', colWidths=[0.1] * len(matrix[0]))
    table.auto_set_font_size(False)
    table.set_fontsize(10)
    plt.show()

#Training data itself is having only 41.477% Accuracy!
...
# b is going to store the estimated parameters from the GMM
# for each of the classes
# Estimated parameters for a GMM are vector_parameters
# Each element of b is a dict
...
def lg(R,S):
    #This function takes in the data array S
    #Parameters of GMM is taken as R
...
def GMM_update(m,g): #Updates b thats all
    ...
    for q in range(k):
        ...
        for s in range(m,m+1):
            ...
            # This is a (ck,d) array
            # We can directly access it to extract out the initial mean!
            # Assign each data point to a cluster
            clusters, _ = vq(data_array_c, centroids) 
            # vq is vector quantisation!
            # This is a 1-d array of (N,) of [0/1]
            # Centroids[0] has mean of 1st clustering
            # Its number is given as 0 in clusters!
            # Covariance have to be computed
            ...
            for i in range(0, M):
                ...
                variable_name = f"v_{i}" #Variable name is a string
                # Create a vector
                # GMM coeff
                count = 0
                for z in clusters:
            ...
            # Lets create some space for storing posterior probabilities
            # They depend on data point
            post_prob = np.zeros((N,M))
            #defined outside the loop as likelihood_calc3
        ...
        #We choose the optimal hyperparameter for each class
        # max_index = Final_Likelihood.index(max(Final_Likelihood))
        b[q] = vector_parameters
     
#Hurray the algorithm is working
#Even after initialising with K-means huge amount of data
...
for s in range(5,6):
    print(loss3(s,40)[0])
    print(loss_training3())
    
#Training data accuracy is 51.695%

#So we can also vary the Hyper Paramter M keeping g which #EM iterations fixed
#Each time we would get a different Likelihood curve for each of the class-conditional density estimation
#Either we can choose M based on max_p after g iterations 
#Or we can go one step further and directly choose the M that gives max_accuracy

#For each of class conditional density estimation how do we come up with the optimal M?
#M=5 gives 68.38 and 71.66222222222223
...
def Bayes_class5(x:np.ndarray,Y:np.ndarray,Z:np.ndarray,z:int):
    ...
    for s in range(knn):
        ...
        p[label-1] += 1
    #We have to account for class priors as well.We can ignore k in denominator.
...
def loss5(z):
    ...
    Loss = 0 ; CM = np.zeros((k,k))
    C = 350 #Computationally Intensive
    for r in range(0,C):
        X = test_data_f[r]
        Y = int(test_data_l[r])
...
# For 25000 test samples AccuracyE1 = 8.472%
